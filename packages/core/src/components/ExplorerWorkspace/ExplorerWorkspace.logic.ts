...
// --- Drag-to-split zone calculation ---

export type DropZone = 'left' | 'right' | 'top' | 'bottom';

/**
 * Given the editor bounding rect and a drag cursor position, returns
 * which edge zone the cursor is closest to.
 */
export function getEditorDropTarget(rect: DOMRect, clientX: number, clientY: number): DropZone {
...
// --- Keyboard shortcut decision logic ---

export type ShortcutAction = 'add-tab' | 'close-tab' | 'close-tab-confirm' | null;

/**
 * Pure decision function for tab keyboard shortcuts (Cmd/Ctrl+T and Cmd/Ctrl+W).
 * Returns the action to take, or null if the event is not a known shortcut.
 * The component is still responsible for preventDefault and dispatch.
 */
export function getShortcutAction(e: { metaKey: boolean; ctrlKey: boolean; key: string }, tabCount: number, activeTabKql: string): ShortcutAction {
...
export function getQueryToRun(
    getSelection: () => { isEmpty: () => boolean; startLineNumber: number; endLineNumber: number } | null, getValueInRange: (sel: { startLineNumber: number; endLineNumber: number }) => string, getPosition: () => { lineNumber: number } | null, getLineCount: () => number, getLineContent: (line: number) => string): QueryRange | null {
 ...
 */
export function getQueryToRunFromEditor(ed: { getSelection: () => unknown; getModel: () => unknown; getPosition: () => unknown }): QueryRange | null {
