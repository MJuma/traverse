...
describe('nextQueryTitle', () => {
    it('returns Query 1 when no tabs exist', () => {
        expect(nextQueryTitle([])).toBe('Query 1');
    });

    it('returns next available number', () => {
        const tabs = [createTab('', 'Query 1'), createTab('', 'Query 2')];
        expect(nextQueryTitle(tabs)).toBe('Query 3');
    });

    it('fills gaps in numbering', () => {
        const tabs = [createTab('', 'Query 1'), createTab('', 'Query 3')];
        expect(nextQueryTitle(tabs)).toBe('Query 2');
    });

    it('ignores non-default titles', () => {
        const tabs = [createTab('', 'My Custom Tab'), createTab('', 'Query 3')];
...
describe('getEditorDropTarget', () => {
    const rect = { left: 100, top: 100, width: 400, height: 300 } as DOMRect;

    it('returns "left" when cursor is closest to the left edge', () => {
        // relX=0.05, relY=0.5, distLeft=0.05 is smallest
        expect(getEditorDropTarget(rect, 120, 250)).toBe('left');
    });

    it('returns "right" when cursor is closest to the right edge', () => {
        // relX=0.95, relY=0.5, distRight=0.05 is smallest
        expect(getEditorDropTarget(rect, 480, 250)).toBe('right');
    });

    it('returns "top" when cursor is closest to the top edge', () => {
        // relX=0.5, relY=0.05, distTop=0.05 is smallest
        expect(getEditorDropTarget(rect, 300, 115)).toBe('top');
    });

    it('returns "bottom" when cursor is closest to the bottom edge', () => {
        // relX=0.5, relY=0.95, distBottom=0.05 is smallest
        expect(getEditorDropTarget(rect, 300, 385)).toBe('bottom');
    });

    it('returns "left" for top-left corner (left wins tie-break)', () => {
        // Near top-left: distLeft and distTop are both small but left is checked first
        expect(getEditorDropTarget(rect, 105, 105)).toBe('left');
    });

    it('handles cursor at exact center', () => {
        // All distances equal at 0.5 — left wins due to Math.min returning first minimum
        const result = getEditorDropTarget(rect, 300, 250);
...
describe('getShortcutAction', () => {
    it('returns "add-tab" for Cmd+T', () => {
        expect(getShortcutAction({ metaKey: true, ctrlKey: false, key: 't' }, 2, '')).toBe('add-tab');
    });

    it('returns "add-tab" for Ctrl+T', () => {
        expect(getShortcutAction({ metaKey: false, ctrlKey: true, key: 't' }, 2, '')).toBe('add-tab');
    });

    it('returns "close-tab" for Cmd+W with empty kql and multiple tabs', () => {
        expect(getShortcutAction({ metaKey: true, ctrlKey: false, key: 'w' }, 2, '')).toBe('close-tab');
    });

    it('returns "close-tab-confirm" for Cmd+W with content', () => {
        expect(getShortcutAction({ metaKey: true, ctrlKey: false, key: 'w' }, 2, 'some kql')).toBe('close-tab-confirm');
    });

    it('returns "close-tab-confirm" for Cmd+W with whitespace-padded content', () => {
        expect(getShortcutAction({ metaKey: true, ctrlKey: false, key: 'w' }, 2, '  query  ')).toBe('close-tab-confirm');
    });

    it('returns null for Cmd+W with only 1 tab', () => {
        expect(getShortcutAction({ metaKey: true, ctrlKey: false, key: 'w' }, 1, '')).toBeNull();
    });

    it('returns null for non-shortcut key', () => {
        expect(getShortcutAction({ metaKey: true, ctrlKey: false, key: 'a' }, 2, '')).toBeNull();
    });

    it('returns null when neither metaKey nor ctrlKey is pressed', () => {
        expect(getShortcutAction({ metaKey: false, ctrlKey: false, key: 't' }, 2, '')).toBeNull();
    });

    it('returns null for Cmd+W with whitespace-only kql and 1 tab', () => {
        expect(getShortcutAction({ metaKey: true, ctrlKey: false, key: 'w' }, 1, '   ')).toBeNull();
    });
});

// ---------------------------------------------------------------------------
// nextQueryTitle (additional edge cases)
// ---------------------------------------------------------------------------

describe('nextQueryTitle (edge cases)', () => {
    it('returns Query 1 when all tabs have custom titles', () => {
        const tabs = [createTab('', 'Custom A'), createTab('', 'Custom B')];
        expect(nextQueryTitle(tabs)).toBe('Query 1');
    });

    it('handles large gap in numbering', () => {
        const tabs = [createTab('', 'Query 100')];
        expect(nextQueryTitle(tabs)).toBe('Query 1');
    });

    it('handles consecutive numbers starting from 2', () => {
        const tabs = [createTab('', 'Query 2'), createTab('', 'Query 3')];
        expect(nextQueryTitle(tabs)).toBe('Query 1');
    });
});
