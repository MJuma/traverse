...
export function ExplorerWorkspace(props: ExplorerProps) {
    ...
    const handlePrimaryDragOver = useCallback((e: React.DragEvent) => {
        if (!dragTabIdRef.current || dragTabIdRef.current === activeTabId) {
            return;
        }
        e.preventDefault();
        ...
        if (minDist === distLeft) {
            setEditorDropTarget('left');
        }
        else if (minDist === distRight) {
            setEditorDropTarget('right');
        }
        else if (minDist === distTop) {
            setEditorDropTarget('top');
        }
        else setEditorDropTarget('bottom');
    ...
    const handlePrimaryDrop = useCallback(() => {
        if (dragTabIdRef.current && dragTabIdRef.current !== activeTabId && editorDropTarget) {
            ...
            dispatch({ type: 'TOGGLE_SPLIT', direction: dir });
