...
import { parseSchema, loadSchema, reloadSchema, getSchema, getSchemaFolders, listDatabases, KQL_FUNCTIONS, KQL_AGGREGATIONS, KQL_SCALAR_FUNCTIONS } from './schema';
...
describe('parseSchema', () => {
    ...
    it('defaults folder to "Other" for tables without Folder', () => {
        const db: KustoSchemaDb = {
            Tables: {
                NoFolder: {
                    DocString: 'No folder',
    ...
    it('defaults folder to "Views" for materialized views without Folder', () => {
        const db: KustoSchemaDb = {
            Tables: {},
            MaterializedViews: {
                NoFolder: {},
        ...
        const result = parseSchema(db);
        expect(result[0].folder).toBe('Views');
    });

    it('defaults folder to "Functions" for functions without Folder', () => {
        const db: KustoSchemaDb = {
            Tables: {},
            Functions: {
                NoFolder: {},
        ...
        const result = parseSchema(db);
        expect(result[0].folder).toBe('Functions');
    });
