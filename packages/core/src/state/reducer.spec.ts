...
describe('explorerReducer', () => {
    ...
    describe('CLOSE_OTHER_TABS', () => {
        it('keeps only the specified tab', () => {
            ...
            const s1 = explorerReducer(initial, { type: 'ADD_TAB' });
            const s2 = explorerReducer(s1, { type: 'ADD_TAB' });
            const keptId = s2.tabs[1].id;
            const state = explorerReducer(s2, { type: 'CLOSE_OTHER_TABS', tabId: keptId });
            expect(state.tabs).toHaveLength(1);
            expect(state.tabs[0].id).toBe(keptId);
            expect(state.activeTabId).toBe(keptId);
            expect(state.splitEnabled).toBe(false);
        });
    });

    describe('CLOSE_ALL_TABS', () => {
        it('replaces all tabs with a fresh tab', () => {
            const initial = makeState();
            const s1 = explorerReducer(initial, { type: 'ADD_TAB' });
            const state = explorerReducer(s1, { type: 'CLOSE_ALL_TABS' });
            expect(state.tabs).toHaveLength(1);
    ...
    describe('DUPLICATE_TAB', () => {
        it('duplicates a tab with same KQL and connection', () => {
            const initial = makeState();
            const tabId = initial.tabs[0].id;
            const updated = explorerReducer(initial, { type: 'UPDATE_TAB_KQL', tabId, kql: 'test query' });
            const state = explorerReducer(updated, { type: 'DUPLICATE_TAB', tabId });
            expect(state.tabs).toHaveLength(2);
            expect(state.tabs[1].kql).toBe('test query');
            expect(state.tabs[1].connectionId).toBe(initial.tabs[0].connectionId);
            expect(state.tabs[1].id).not.toBe(tabId);
            expect(state.activeTabId).toBe(state.tabs[1].id);
        });

        it('inserts duplicate after the source tab', () => {
            const initial = makeState();
            const s1 = explorerReducer(initial, { type: 'ADD_TAB' });
            const firstId = s1.tabs[0].id;
            const state = explorerReducer(s1, { type: 'DUPLICATE_TAB', tabId: firstId });
            expect(state.tabs).toHaveLength(3);
            expect(state.tabs[0].id).toBe(firstId);
            expect(state.tabs[1].kql).toBe(s1.tabs[0].kql);
        });
    });
